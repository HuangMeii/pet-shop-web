...
export default function ProductsPage() {
  ...
  const [recommendedPets, setRecommendedPets] = useState<PetResponse[]>([]);
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          // Map recommended product IDs to full product data
          ...
          // If no recommended products from server, fall back to random products
          setRecommendedProducts(
            mappedProducts.length > 0
              ? mappedProducts
              : productsData.sort(() => Math.random() - 0.5).slice(0, 8)
          );
          setRecommendedPets(
            recData.pets
          ...
          setRecError("Không thể tải gợi ý");
        } finally {
          setRecLoading(false);
        }
      } catch (err: unknown) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <RecommendationSection
                ...
                error={recError}
                maxItems={3}
                viewAllLink="/user/recommendations"
              />
              <RecommendationSection
                title="🐾 Thú cưng gợi ý"
                type="pet"
                items={recommendedPets}
                loading={recLoading}
                error={recError}
                maxItems={3}
                viewAllLink="/user/recommendations"
