...
export function useReview(): UseReviewReturn {
  ...
  const placeOrder = useCallback(async () => {
    ...
    try {
      ...
      // Nếu chọn QR_Scanning (PayOS), tạo link thanh toán và redirect
      if (paymentMethod === "QR_Scanning") {
        const totalAmount = Math.round(invoice.realAmount ?? invoice.totalAmount ?? 0);
        const paymentLink = await createPaymentLink({
          amount: totalAmount,
          description: `HappyPetShop - ${invoice.id}`,
          orderId: invoice.id,
